...
class Address(object):
	def __init__(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0):
		...
		if seed:
			...
			if not waves.validateAddress(address):
				...
				self.seed = seed
				self.nonce = nonce
		elif alias and not waves.OFFLINE:
			self.address = waves.wrapper('/alias/by-alias/%s' % alias).get("address", "")
			self.publicKey = ''
			self.privateKey = ''
			self.seed = ''
			self.nonce = 0
	...
	def __str__(self):
		if self.address:
			ab = []
			try:
				assets_balances = waves.wrapper('/assets/balance/%s' % self.address)['balances']
	...
	def assets(self):
		req = waves.wrapper('/assets/balance/%s' % self.address)['balances']
		return [r['assetId'] for r in req]

	def aliases(self):
		a = waves.wrapper('/alias/by-address/%s' % self.address)
		if type(a)==list:
	...
	def _generate(self, publicKey='', privateKey='', seed='', nonce=0):
		...
		unhashedAddress = chr(1) + str(waves.CHAIN_ID) + crypto.hashChain(pubKey)[0:20]
	...
	def issueAsset(self, name, description, quantity, decimals=0, reissuable=False, txFee=waves.DEFAULT_ASSET_FEE):
		if not self.privateKey:
			msg = 'Private key required'
			logging.error(msg)
			waves.throw_error(msg)
			...
			req = waves.wrapper('/assets/broadcast/issue', data)
			if waves.OFFLINE:
				return req
			else:
				return waves.Asset(req['assetId'])

	def reissueAsset(self, Asset, quantity, reissuable=False, txFee=waves.DEFAULT_TX_FEE):
		timestamp = int(time.time() * 1000)
		sData = b'\5' + \
				base58.b58decode(self.publicKey) + \
		...
		req = waves.wrapper('/assets/broadcast/reissue', data)
		if waves.OFFLINE:
			return req
		else:
			return req.get('id', 'ERROR')

	def burnAsset(self, Asset, quantity, txFee=waves.DEFAULT_TX_FEE):
		timestamp = int(time.time() * 1000)
		...
		req = waves.wrapper('/assets/broadcast/burn', data)
		if waves.OFFLINE:
			return req
		else:
			return req.get('id', 'ERROR')

	def sendWaves(self, recipient, amount, attachment='', txFee=waves.DEFAULT_TX_FEE, timestamp=0):
		if not self.privateKey:
			msg = 'Private key required'
			logging.error(msg)
			waves.throw_error(msg)
			...
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and self.balance() < amount + txFee:
			...
			sig = signature.decode('ascii') #decode('unicode_escape') #decode('utf-8')
			print('signature:',sig)
			print('self.publicKey:',self.publicKey)
			pk = self.publicKey.decode('ascii')
			print('self.publicKey:',pk)
			print('recipient.address:',recipient.address)
			r = recipient.address
			print('recipient.address:',r)
			data = json.dumps({
				"senderPublicKey": pk,
				"recipient": r,
				"amount": amount,
				"fee": txFee,
				"timestamp": timestamp,
				"attachment": base58.b58encode(crypto.str2bytes(attachment)).decode('ascii'),
				"signature": sig
			})
			'''
			data = json.dumps({
				"senderPublicKey": self.publicKey.decode('ascii'),
			...
			return waves.wrapper('/assets/broadcast/transfer', data)
	...
	def massTransferWaves(self, transfers, attachment='', timestamp=0):
		...
		if not self.privateKey:
			...
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and self.balance() < totalAmount + txFee:
			msg = 'Insufficient Waves balance'
			logging.error(msg)
			waves.throw_error(msg)
		else:
			...
			return waves.wrapper('/transactions/broadcast', data)

	def sendAsset(self, recipient, asset, amount, attachment='', feeAsset='', txFee=waves.DEFAULT_TX_FEE, timestamp=0):
		if not self.privateKey:
			msg = 'Asset not issued'
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and asset and not asset.status():
			msg = 'Asset not issued'
			logging.error(msg)
			waves.throw_error(msg)
		elif amount <= 0:
			...
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and asset and self.balance(asset.assetId) < amount:
			msg = 'Insufficient asset balance'
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and not asset and self.balance() < amount:
			msg = 'Insufficient Waves balance'
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and not feeAsset and self.balance() < txFee:
			msg = 'Insufficient Waves balance'
			logging.error(msg)
			waves.throw_error(msg)
		elif not waves.OFFLINE and feeAsset and self.balance(feeAsset.assetId) < txFee:
			msg = 'Insufficient asset balance'
			logging.error(msg)
			waves.throw_error(msg)
		else:
	...
	def massTransferAssets(self, transfers, asset, attachment='', timestamp=0):
		...
		elif not waves.OFFLINE and self.balance() < txFee:
			logging.error('Insufficient Waves balance')
		elif not waves.OFFLINE and self.balance(assetId=asset.assetId) < totalAmount:
	...
	def dataTransaction(self, data, timestamp=0):
		if not self.privateKey:
			...
			return waves.wrapper('/transactions/broadcast', dataObjectJSON)

	def _postOrder(self, amountAsset, priceAsset, orderType, amount, price, maxLifetime=30*86400, matcherFee=waves.DEFAULT_MATCHER_FEE, timestamp=0):
		if timestamp == 0:
			timestamp = int(time.time() * 1000)
		...
		sData = base58.b58decode(self.publicKey) + \
				base58.b58decode(waves.MATCHER_PUBLICKEY) + \
				asset1 + \
				asset2 + \
				orderType + \
				struct.pack(">Q", price) + \
		...
		data = json.dumps({
			"senderPublicKey": self.publicKey,
			"matcherPublicKey": waves.MATCHER_PUBLICKEY,
			"assetPair": {
		...
		req = waves.wrapper('/matcher/orderbook', data, host=waves.MATCHER)
		id = -1
		if 'status' in req:
		...
		elif not waves.OFFLINE:
			logging.error(req)
			waves.throw_error(req)
		else:
			return req
		return id
	...
	def cancelOrder(self, assetPair, order):
		...
		req = waves.wrapper('/matcher/orderbook/%s/%s/cancel' % ('WAVES' if assetPair.asset1.assetId=='' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId=='' else assetPair.asset2.assetId), data, host=waves.MATCHER)
		if waves.OFFLINE:
			return req
		else:
			id = -1
	...
	def buy(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=waves.DEFAULT_MATCHER_FEE, timestamp=0):
		assetPair.refresh()
		...
		if waves.OFFLINE:
			...
			return waves.Order(id, assetPair, self)

	def sell(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=waves.DEFAULT_MATCHER_FEE, timestamp=0):
		assetPair.refresh()
		normPrice = int(pow(10, assetPair.asset2.decimals - assetPair.asset1.decimals) * price)
	...
	def tradableBalance(self, assetPair):
		try:
			req = waves.wrapper('/matcher/orderbook/%s/%s/tradableBalance/%s' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId, self.address), host=waves.MATCHER)
			if waves.OFFLINE:
					return req
	...
	def lease(self, recipient, amount, txFee=waves.DEFAULT_LEASE_FEE, timestamp=0):
		if not self.privateKey:
			msg = 'Private key required'
			logging.error(msg)
			waves.throw_error(msg)
			...
			req = waves.wrapper('/leasing/broadcast/lease', data)
			return req

	def leaseCancel(self, leaseId, txFee=waves.DEFAULT_LEASE_FEE, timestamp=0):
		if not self.privateKey:
			msg = 'Private key required'
			logging.error(msg)
			waves.throw_error(msg)
			...
			req = waves.wrapper('/leasing/broadcast/cancel', data)
			if waves.OFFLINE:
				return req
			elif 'leaseId' in req:
				return req['leaseId']
	...
	def getOrderHistory(self, assetPair, timestamp=0):
		...
		req = waves.wrapper('/matcher/orderbook/%s/%s/publicKey/%s' % ('WAVES' if assetPair.asset1.assetId=='' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId=='' else assetPair.asset2.assetId, self.publicKey), headers=data, host=waves.MATCHER)
		return req
	...
	def cancelOpenOrders(self, assetPair):
		...
		for order in orders:
			...
			if status=='Accepted' or status=='PartiallyFilled':
				...
				waves.wrapper('/matcher/orderbook/%s/%s/cancel' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=waves.MATCHER)
	...
	def deleteOrderHistory(self, assetPair):
		...
		for order in orders:
			...
			waves.wrapper('/matcher/orderbook/%s/%s/delete' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=waves.MATCHER)

	def createAlias(self, alias, txFee=waves.DEFAULT_ALIAS_FEE, timestamp=0):
		aliasWithNetwork = b'\x02' + crypto.str2bytes(str(waves.CHAIN_ID)) + struct.pack(">H", len(alias)) + crypto.str2bytes(alias)
		if not self.privateKey:
			msg = 'Private key required'
			logging.error(msg)
			waves.throw_error(msg)
			...
			return waves.wrapper('/alias/broadcast/create', data)

	def sponsorAsset(self, assetId, minimalFeeInAssets, txFee=waves.DEFAULT_SPONSOR_FEE, timestamp=0):
		if not self.privateKey:
			logging.error('Private key required')
		else:
			if timestamp == 0:
	...
	def setScript(self, scriptSource, txFee=waves.DEFAULT_SCRIPT_FEE, timestamp=0):
		script = waves.wrapper('/utils/script/compile', scriptSource)['script'][7:]
		if not self.privateKey:
			logging.error('Private key required')
		else:
			...
			sData = b'\x0d' + \
				b'\1' + \
				crypto.str2bytes(str(waves.CHAIN_ID)) + \
				base58.b58decode(self.publicKey) + \
				b'\1' + \
