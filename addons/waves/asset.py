...
class Asset(object):
	...
	def status(self):
		if self.assetId!='WAVES':
			try:
				req = waves.wrapper('/transactions/info/%s' % self.assetId)
				if req['type'] == 3:
...
class AssetPair(object):
	...
	def orderbook(self):
		req = waves.wrapper('/matcher/orderbook/%s/%s' % (self.a1, self.a2), host=waves.MATCHER)
		return req

	def ticker(self):
		return waves.wrapper('/api/ticker/%s/%s' % (self.a1, self.a2), host=waves.DATAFEED)
	...
	def _getMarketData(self, method, params):
		return waves.wrapper('%s/%s/%s/%s' % (method, self.a1, self.a2, params), host=waves.DATAFEED)

	def trades(self, *args):
		if len(args)==1:
			limit = args[0]
			if limit > 0 and limit <= waves.MAX_WDF_REQUEST:
	...
	def candles(self, *args):
		if len(args)==2:
			timeframe = args[0]
			limit = args[1]
			if timeframe not in waves.VALID_TIMEFRAMES:
				msg = 'Invalid timeframe'
				waves.throw_error(msg)
				return logging.error(msg)
			elif limit > 0 and limit <= waves.MAX_WDF_REQUEST:
